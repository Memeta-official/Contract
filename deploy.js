...
console.log(`--------------------------------------------------`);
console.log(`NETWORK: ${hre.network.name} | PROTOCOL_BENEFICIARY: ${PROTOCOL_BENEFICIARY}`);
...
async function main() {
  ...
  console.log(
    `Deploy from account: ${deployer} - Balance: ${Number(await hre.ethers.provider.getBalance(deployer)) / 1e18} ETH`
  );
